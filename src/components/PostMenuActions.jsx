...
const PostMenuActions = ({ post }) => {
  ...
  return (
    <div>
      {user ? (<h1 className="mt-8 mb-4 text-sm font-medium">Actions</h1>) : (<h1 className="mt-8 mb-4 text-sm font-medium">Login for more actions</h1>)}
      ...
      {user && (
        ...
        >
          <path
            ...
            stroke="black"
            strokeWidth="2"
            fill={isSaved ? "skyblue" : "none"}
          />
        </svg>
        <span>Save this Post</span>
        {saveMutation.isPending && !isSaved && <span className="text-xm text-sky-500">saving</span>}
      ...
      {isAdmin && (<div className="flex items-center gap-2 py-2 text-sm cursor-pointer" onClick={handleFeature}>
        ...
        {featureMutation.isPending && !post.isFeatured && (<span className="text-xm text-yellow-500">featuring</span>)}
      </div>)}
     ...
     {user && (post.user.username === user.username || isAdmin) && (
      <div className="flex items-center gap-2 py-2 text-sm cursor-pointer" onClick={handleDelete}>
        ...
        {deleteMutation.isPending && <span className="text-xm text-red-500">deleting</span>}
      </div>)}
